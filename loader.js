  ...
  MAMERunner.prototype = Object.create(EmscriptenRunner.prototype,
    {
      mute: {
        value: function () {
          var jsmame = window.JSMAME || window.JSMESS || {};
          if (jsmame.sound_manager_mute && jsmame.get_sound && jsmame.get_machine) {
            var machine = jsmame.get_machine();
            var soundmgr = jsmame.get_sound(machine);
            jsmame.sound_manager_mute(soundmgr, true, 0x02); // MUTE_REASON_UI
          } else if (typeof Module !== 'undefined' && Module && Module._ZN13sound_manager4muteEbh) {
            var machine = Module._ZN15running_machine30emscripten_get_running_machineEv();
            var soundmgr = Module._ZN15running_machine20emscripten_get_soundEv(machine);
            Module._ZN13sound_manager4muteEbh(soundmgr, true, 0x02);
          }
        },
      },
      unmute: {
        value: function () {
          ...
          if (jsmame.sound_manager_mute && jsmame.get_sound && jsmame.get_machine) {
            var machine = jsmame.get_machine();
            var soundmgr = jsmame.get_sound(machine);
            jsmame.sound_manager_mute(soundmgr, false, 0x02); // MUTE_REASON_UI
          } else if (typeof Module !== 'undefined' && Module && Module._ZN13sound_manager4muteEbh) {
            var machine = Module._ZN15running_machine30emscripten_get_running_machineEv();
            var soundmgr = Module._ZN15running_machine20emscripten_get_soundEv(machine);
            Module._ZN13sound_manager4muteEbh(soundmgr, false, 0x02);
          }
        },
      },
    });
  ...
  function Emulator(canvas, callbacks, loadFiles) {
    ...
    this.setMute = function (state) {
      ...
      if (runner) {
        ...
        try {
          if (typeof Module !== 'undefined' && Module && Module.cwrap) {
            if (!window.SDL_PauseAudio)
              window.SDL_PauseAudio = Module.cwrap('SDL_PauseAudio', '', ['number']);
