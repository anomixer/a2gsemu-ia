...
#### 選單自動滾動與滑鼠鎖定
- **功能**：點選遊戲時立即滾動到置中位置，支援滑鼠鎖定
- **實現**：精確計算 `scrollTop = itemOffsetTop - (listHeight / 2) + (itemHeight / 2)`
- **記憶**：使用 `sessionStorage` 記住滾動位置，頁面載入時自動恢復
- **滑鼠鎖定**：點擊 canvas 鎖定滑鼠，按 Esc 解鎖，標題列顯示黃色提示

#### 解析度與顯示優化
- **原生解析度**：704x462，無邊框設計
- **全螢幕模式**：按 704:462 比例等比例縮放，保持像素完美
- **截圖顯示**：滿版顯示，提供最佳預覽效果
- **像素渲染**：使用 `image-rendering: pixelated` 保持復古效果
